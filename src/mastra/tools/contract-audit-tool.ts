...
export const contractAuditTool = createTool({
  id: 'audit-contract',
  description: 'Audit enterprise contracts for visualization dashboard compliance',
  inputSchema: z.object({
    contractContent: z.string().describe('Contract content text to audit'),
...
const generateSummary = (score: number, riskLevel: RiskLevel, contractType: ContractType): string => {
  ...
  return `${typeMap[contractType]}合同合规审核完成。合规得分：${score}分，风险等级：${riskMap[riskLevel]}。${
    score >= 85 ? '合同整体合规性良好。' : 
